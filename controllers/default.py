...
def scanner():
				...
				if request.vars.delete == 'all':
						for s in session.scan:
								try:
										...
										hiba += " %s" % str( e )
						if hiba == '':
								response.flash = T( 'Scans deleted!' )
						else:
		...
		if session.scan:
				msg = DIV( A( T( 'Delete scans' ), _href = URL( r = request, f = 'scanner', vars = dict( delete = 'all' ) ) ),
								P( A( T( 'Next scan' ),
										_id = 'scanlink',
										_href = URL( r = request,
															f = 'scanner',
															vars = dict( scan = 1 ) ), _style = 'text:center;' ) ) )
		else:
				msg = DIV( P( A( T( 'Scan' ), _id = 'scanlink', _href = URL( r = request,
																	f = 'scanner',
																	vars = dict( scan = 1 ) ) ) ) )
		...
		if not session.scan and not request.vars.scan:
				session.scan = []
				msg = DIV( P( T( 'Put the paper on the scanner!' ) ), P( A( T( 'Scan' ),
																				_id = 'scanlink',
																				_href = URL( r = request,
																									f = 'scanner',
																									vars = dict( scan = 1 ) ) ) ) )
				return dict( msg = msg, form = form )
		if request.vars.scan == '1':
				import time
				import subprocess
				scanconfig = db( db.scannerconfig.auth_user_id == auth.user_id ).select().first()
				imagename = "scan-%s" % time.time()
				sfname = "%s.tif" % imagename
				scanfile = os.path.join( request.folder, 'static', sfname )
				cmd = [config.prgscanimage, '--format=tiff',
				...
				if proc.returncode:
						...
						se.seek( 0 )
						response.flash = T( 'Error in scanning paper. Try again! [ %(error)s]',
														 dict( error = str( e ) ) )
				...
				try:
						...
						response.flash = T( "Error in conversion: %(error)s", dict( error = str( e ) ) )
						try:
								os.unlink( scanjpg )
								os.unlink( thumbjpg )
						except:
